...
// ---- landmarks and waystones ----
export function bakeWaystone(lit) { const [c, g] = canvas(16, 24); rect(g, 4, 6, 8, 16, D.rock.face); rect(g, 5, 4, 6, 3, D.rock.faceL); rect(g, 4, 6, 1, 16, D.rock.faceL); rect(g, 11, 6, 1, 16, D.rock.faceD); ellipse(g, 8, 22, 7, 2, D.rock.faceD); const rune = lit ? '#7ad0ff' : '#3a3140'; rect(g, 7, 9, 2, 8, rune); rect(g, 5, 12, 6, 2, rune); px(g, 6, 10, rune); px(g, 9, 10, rune); if (lit) { px(g, 7, 8, '#e0f6ff'); } outline(c, '#241826'); return { canvas: c, ax: 8, ay: 23 }; }
export function bakeTotem() { const [c, g] = canvas(16, 40); rect(g, 7, 12, 3, 27, D.wood); rect(g, 7, 12, 1, 27, D.woodL); circle(g, 8, 9, 5, D.bone); px(g, 6, 8, '#1b1626'); px(g, 10, 8, '#1b1626'); rect(g, 6, 11, 5, 1, D.boneD); px(g, 7, 12, D.boneD); px(g, 9, 12, D.boneD); for (let i = 0; i < 3; i++) { line(g, 3 + i * 4, 3, 4 + i * 4, 1 - (i & 1), i === 1 ? '#c9452e' : '#e0bb65', 1); } rect(g, 3, 20, 10, 2, '#7a3a2a'); rect(g, 4, 26, 8, 1, '#c9a56a'); ellipse(g, 8, 39, 6, 2, D.earth.deep); outline(c, '#241826'); return { canvas: c, ax: 8, ay: 39 }; }
export function bakeBanner(col) { const [c, g] = canvas(14, 34); rect(g, 6, 2, 2, 31, D.wood); rect(g, 6, 2, 1, 31, D.woodL); rect(g, 2, 3, 10, 1, D.woodL); rect(g, 3, 4, 8, 18, col); rect(g, 3, 4, 2, 18, shade(col, 0.2)); rect(g, 9, 4, 2, 18, shade(col, -0.3)); for (let i = 0; i < 4; i++) rect(g, 3 + i * 2, 21 + (i & 1), 2, 2, col); circle(g, 7, 12, 2.5, D.bone); px(g, 6, 11, '#1b1626'); px(g, 8, 11, '#1b1626'); outline(c, '#241826'); return { canvas: c, ax: 7, ay: 33 }; }
export function bakeCage() { const [c, g] = canvas(20, 26); rect(g, 2, 4, 16, 20, 'rgba(20,14,30,0.6)'); for (let x = 2; x <= 18; x += 4) rect(g, x, 3, 2, 21, D.iron); rect(g, 1, 2, 18, 2, D.iron); rect(g, 1, 12, 18, 1, D.iron); rect(g, 1, 23, 18, 2, D.ironL); px(g, 2, 3, D.ironL); px(g, 6, 3, D.ironL); px(g, 10, 3, D.ironL); circle(g, 9, 14, 3, D.bone); px(g, 8, 13, '#1b1626'); px(g, 10, 13, '#1b1626'); line(g, 4, 20, 14, 19, D.boneD); outline(c, '#241826'); return { canvas: c, ax: 10, ay: 25 }; }
export function bakeSkulls(seed) { const rnd = mulberry(seed); const [c, g] = canvas(22, 14); for (let i = 0; i < 6; i++) { const x = 3 + Math.floor(rnd() * 15), y = 5 + Math.floor(rnd() * 6); circle(g, x, y, 2.6, D.bone); px(g, x - 1, y, D.boneD); px(g, x + 1, y, D.boneD); px(g, x, y + 2, D.boneD); } line(g, 2, 12, 12, 11, D.boneD); line(g, 10, 13, 19, 12, D.bone); outline(c, '#241826'); return { canvas: c, ax: 11, ay: 13 }; }
